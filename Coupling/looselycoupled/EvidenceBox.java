...
public class EvidenceBox {
    ...
    /**
     * This is a method, when invoked, will receive the type value from another another method in police class and check if the type is the same to the evidences
     * @param type This is a parameter that will be passed from another method in police class
     * @return This will return the list of evidences that had the same type
     */
    public ArrayList<Evidence> getEvidenceByType(String type)  {
        ArrayList<Evidence> evidencePieces = new ArrayList<>();
        for( Evidence e : extract() ) {
            // Checking if the type matched the requested type
            if( e.getType().equals(type) ) {
                evidencePieces.add(e);
            }
        }
